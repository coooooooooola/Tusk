...
import com.meituan.ep.leo.dispatch.DispatchConf;
import com.meituan.ep.leo.toolkit.util.CommUtils;
import com.meituan.ep.leo.toolkit.util.FileUtil;
...
public class CreateXmlFile {
    ...
    public void addJavaCase(String caseName, String pkgAndClsName) {
        if (null==pkgAndClsName ) {
            log.error("添加的java类为null，添加失败");
            return;
        }
        if (null==caseName || caseName.length()<1) {
            ...
            String nameTmp="未命名测试集"+CommUtils.getRandomStr(5);
            log.info("使用默认测试集名称："+nameTmp);
            addClassToXmlTest(pkgAndClsName, nameTmp);
            return;
        }
        addClassToXmlTest(pkgAndClsName, caseName);
    }
    ...
    public void addListener(String listenerPath){
        if(listenerPath == null){
            log.error("listener为空");
            return;
        }
        suite.addListener(listenerPath);
    }
...
//
//    public void addClassToxml(String pkgAndClsName,String testName){
//        this.addClassToXmlTest(pkgAndClsName, testName);
//    }
