...
//        	log.info("此目录下未取得任何文件："+folderPath);
	...
	/**
	 * 指定编码方式，读取properties value ，读取不到则抛出异常
	 * @param key
	 * @param filePath
	 * @param charsetName
	 * @return
	 */
	public static String getProperties(String key, String filePath, String charsetName){
		Properties properties = new Properties();
		String path = filePath;
		InputStreamReader fis;
		try{
			fis = new InputStreamReader(new FileInputStream(filePath), charsetName);
			properties.load(fis);
			fis.close();
		}catch (Exception e){
			log.error(path + " properties路径错误");
			log.error(e.getMessage());
//			System.out.println(path + " properties路径错误");
//			e.printStackTrace();
			return "error";
		}
		return properties.getProperty(key);
